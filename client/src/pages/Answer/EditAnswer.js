...
export const EditAnswer = (props) => {
  ...
  const submitClickHandler = (event) => {
    ...
    if (description.trim() === "") return dispatch(setAlert({ message: "Answer cannot be empty", status: false }));
    
    updateAnswer({description}, answer._id).then((res) => {
  ...
  return (
    <div>
      <textarea onChange={onChangeBody} type="text" value={description}
        className="border-2 border-gray-300 rounded mr-4 mb-30 h-56 p-2 w-11/12 
