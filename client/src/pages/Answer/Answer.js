...
import { setAlert, getAnswers } from "../../redux/actions";
...
const Answer = (props) => {
  ...
  const answerChangeHandler = (event) => { setAns(event.target.value); };

  const answerSubmitHandler = (event) => {
    event.preventDefault();
    
    answerQuestion({ ans, user }, props.question_id).then((res) => {
      ...
      {!loading && !loadingQuestion && answers && (
        <div className="pl-2" style={{ backgroundColor: "rgb(40, 40, 40)"}}>
          ...
          {answers.map((answer, index) => (
            <EachAnswer key={index} answer={answer} user={user} question_id={props.question_id}/>
          ))}
          <div className="text-left text-xl pl-8 p-4 text-white">Your answer</div>
          <textarea onChange={answerChangeHandler} value={ans} type="text"
            className="whitespace-pre-line border-2 border-gray-300 rounded mr-4 mb-30 h-56 p-2 w-11/12 
            focus:border-blue-300 outline-none"/>
          <div className="px-3 flex flex-start">
            <button onClick={answerSubmitHandler}
              className=" p-2 m-4 bg-blue-500 border-2 border-blue-700 rounded text-white 
              hover:bg-blue-600 h-10">Post your Answer
            </button>
          </div>
        </div>
      )}
